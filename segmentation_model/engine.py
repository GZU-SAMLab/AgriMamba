...
def evaluate_combined(model: nn.Module,
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(metric_logger.log_every(data_loader, log_every, header)):
            ...
            if 'leaf_pred' in results and 'leaf_mask' in batch:
                ...
                leaf_mious.append(leaf_metrics['miou'])
                metric_logger.update(leaf_iou=leaf_metrics['iou'])
                metric_logger.update(leaf_miou=leaf_metrics['miou'])
            ...
            if 'lesion_pred' in results and 'lesion_mask' in batch:
                ...
                lesion_mious.append(lesion_metrics['miou'])
                metric_logger.update(lesion_iou=lesion_metrics['iou'])
                metric_logger.update(lesion_miou=lesion_metrics['miou'])
    ...
    if leaf_mious:
        eval_stats['mean_leaf_miou'] = sum(leaf_mious) / len(leaf_mious)
    if lesion_ious:
        eval_stats['mean_lesion_iou'] = sum(lesion_ious) / len(lesion_ious)
    if lesion_mious:
        eval_stats['mean_lesion_miou'] = sum(lesion_mious) / len(lesion_mious)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        if lr_scheduler is not None:
            ...
            try:
                ...
                print(f"Warning: failed to update learning-rate scheduler: {e}")
        ...
        if output_dir and utils.is_main_process():
            ...
            epoch_log = {
                ...
                f"stage{stage}_train_miou": train_stats.get('miou', 0.0),
                ...
                f"stage{stage}_val_miou": val_stats.get('miou', 0.0),
        ...
        if current_miou > best_miou:
            ...
            if output_dir:
                ...
                utils.save_on_master({
                    ...
                    'stage': stage,
                    'best_miou': best_miou,
                    'best_iou': val_stats.get('iou', 0.0),
                    'config': config.to_dict(),
                }, checkpoint_path)
                print(f"Saved best stage {stage} model with mIoU: {best_miou:.3f}")

        print(f"Stage {stage} Epoch {epoch} - Train Loss: {train_stats.get('loss', 0):.4f}, "
              f"Val mIoU: {current_miou:.3f}, Best mIoU: {best_miou:.3f}")

    print(f"Stage {stage} training completed. Best mIoU: {best_miou:.3f}")
